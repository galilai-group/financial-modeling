...
// One navigation model for every section, independent of leaderboard availability.
function setupTabs() {
  const tabs = Array.from(el('content-tabs').querySelectorAll('[role="tab"]'));
  const aliases = {overview: 'leaderboard', examples: 'examples-panel', resources: 'training-section'};
  function show(panel, focus = false) {
    panel = aliases[panel] || panel;
    const selected = tabs.find(tab => !tab.hidden && tab.dataset.panel === panel) || tabs[0];
  ...
  function navigate(tab, focus = false) {
    ...
    const hash = Object.keys(aliases).find(key => key !== 'overview' && aliases[key] === panel) || panel;
    show(panel, focus);
    if (location.hash !== '#' + hash) history.pushState(null, '', '#' + hash);
  }
  for (const tab of tabs) tab.onclick = () => navigate(tab);
  el('content-tabs').onkeydown = event => {
    if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) return;
    event.preventDefault();
    const visible = tabs.filter(tab => !tab.hidden), index = visible.indexOf(document.activeElement);
    const next = event.key === 'Home' ? 0 : event.key === 'End' ? visible.length - 1 :
      (index + (event.key === 'ArrowLeft' ? -1 : 1) + visible.length) % visible.length;
    navigate(visible[next], true);
  };
  window.addEventListener('hashchange', () => show(location.hash.slice(1)));
  window.addEventListener('popstate', () => show(location.hash.slice(1)));
  return () => show(location.hash.slice(1));
}
const refreshTabs = setupTabs();
(async () => {
  try {
    ...
    const fonts = {sans: '-apple-system,BlinkMacSystemFont,"Segoe UI",Helvetica,Arial,sans-serif', serif: 'Georgia,"Times New Roman",serif', mono: 'ui-monospace,SFMono-Regular,Consolas,monospace'};
    document.documentElement.style.fontFamily = fonts[config.font_family] || fonts.sans;
    if (config.example_code?.trim()) {
      el('examples-tab').hidden = false;
    ...
    const dateText = value => new Date(value).toLocaleString(undefined, {year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hourCycle: 'h23', timeZone: 'UTC', timeZoneName: 'short'});
    function schedule() {
    ...
    for (const [key, label] of [['opens_at', 'Submissions open'], ['closes_at', 'Submission deadline'], ['results_at', 'Results announcement']]) {
      ...
      item.append(title, time); el('timeline-dates').append(item); el('timeline-tab').hidden = false;
    }
    ...
    el('group-control').hidden = !(data.groups || []).length;
    el('search').placeholder = anonymous ? 'Search participant ID…' : 'Search participant email…';
    ...
    el('policy').textContent = `Ranked by ${primary} · ${(data.direction || config.direction) === 'minimize' ? 'Lower' : 'Higher'} is better · Latest accepted model per participant · Evaluation ${data.evaluator_version || config.evaluator_version || 'not configured'}`;
    ...
    function render() {
      ...
      el('participant-count').textContent = query || group ? `${rows.length} of ${data.rows.length} participants` : `${data.rows.length} participant${data.rows.length === 1 ? '' : 's'}`;
      el('empty-state').hidden = rows.length > 0;
      el('empty-heading').textContent = data.rows.length ? 'No matching participants' : 'Results will appear here';
      el('empty-description').textContent = data.rows.length ? 'Try a different search or group filter.' : 'Accepted models appear after the organizer evaluates and publishes them.';
      el('status').textContent = '';
    }
    el("search").addEventListener("input", render); el("group-filter").addEventListener("change", render); render();
  } catch { refreshTabs(); el("status").textContent = "Results could not be loaded. Please reload shortly."; }
})();
